defmodule PublicSuffix.TestCaseGenerator do
  ...
  def test_cases do
    ...
    # provides chunks of comment plus test cases matching the comment
...
defmodule PublicSuffixGeneratedCasesTest do
  ...
  test_name = fn test_case, fun_name, output_field ->
    ...
    description =
      "#{test_case.group_description} ##{test_case.group_case_number} -- line #{test_case.line_number}"

    case test_case.group_description do
      "IDN labels" ->
        "#{fun_name} (#{description}) (can't embed expression in test name due to chinese characters)"
